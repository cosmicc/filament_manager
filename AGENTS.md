...
- Render every filament setting from its complete effective template-linked state, visibly highlight explicit customizations, and never show Cura aliases for retract speed or maximum fan speed as overlapping independent controls. Continue emitting deterministic Cura alias values from their single canonical application setting.
