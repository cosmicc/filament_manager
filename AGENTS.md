...
7. Cura files are changed only by the outbound-only per-user workstation agent under `workstation-agent/`; the server never reaches into a workstation or listens on an agent port. After explicit takeover of a workstation with existing user materials, Filament Manager owns the complete user material library, deploys the latest published templates under the exact `Template <material type>` name and `Template` brand plus every published product profile, removes stale user material files after backup, and installs its visibility plugin so bundled Cura materials are hidden. The Cura Material Settings plugin exposes stored values, and the Cura Klipper Settings plugin owns pressure advance and smooth time; never patch machine start G-code or create quality-change profiles.
   ...
   Before authoritative takeover, an Administrator may preserve selected reported Cura materials as source-tracked draft templates or product profiles. Enforce one active `Template <material type>` identity per material family and printer/nozzle scope; additional same-family sources become filament-specific profiles. Takeover must remain blocked until every selected import is active and has a published revision.
...
18. Cura print preflight uses the managed material GUID and the bounded printer-side catalog to bypass a change only when the physical spool matches. Otherwise Fluidd must prompt for one exact eligible spool and confirmation of insertion. Manual `LOAD_FILAMENT`, `FILAMENT_MANAGER_LOAD_TARGET`, and M600 selection use that same live catalog without requiring a separately staged macro variable. Keep `integrations/klipper/filament-manager-macros.cfg` as the complete reference, include it after the printer's existing motion macros, preserve those routines through wrappers, and leave Fluidd's independent print-start spool selector disabled.
...
21. Operator notifications are persistent, deduplicated canonical events with per-user read state. Reappearing resolved conditions become unread. Keep unavailable Moonraker, dead job, low/empty spool, plate maintenance, and failed Cura deployment checks server-side and never expose external response bodies.
...
25. Diagnostics is the single web surface for connection, synchronization, worker, queue, and bounded operational error status. Recovery validation is read-only and results are persisted; projection rebuilds only queue reconstructable Spoolman, Google, and managed Cura work and never mutate canonical business records or perform a live database restore. Never expose credentials, URLs, external response bodies, or tracebacks.
...
- Pre-takeover Cura source provenance is unique across template and product-profile imports. Require one material-family template per printer/nozzle scope and use explicitly selected additional sources for filament-specific draft profiles.
...
- The ordered calibration workflow has seven steps. Size and Hole Calibration is required after Retraction and stores X/Y/Z, hole, shaft, and wall design/measured values plus server-calculated Cura expansion and flow, material shrinkage, and non-applying printer-geometry recommendations; profile publication inherits the complete starting snapshot before applying material results.
