...
test('guided setup preserves exact redirect and uploads credentials without browser storage', async ({ page }, info) => {
  const calls = await googleSettings(page, false, false)
  await page.goto('/settings')
  await page.getByRole('button', { name: 'Guided setup', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Set up Google Sheets' })
  await expect(dialog.getByRole('link', { name: 'Open Google Cloud' })).toBeVisible()
  await dialog.getByRole('button', { name: 'Next', exact: true }).click()
  await expect(dialog.getByRole('link', { name: 'Enable Sheets API' })).toBeVisible()
  await dialog.getByRole('button', { name: 'Next', exact: true }).click()
  await expect(dialog.getByText('https://filament.example.invalid/google/callback', { exact: true })).toBeVisible()
  await page.setViewportSize({ width: 390, height: 844 })
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await page.screenshot({ path: info.outputPath('google-wizard-mobile.png') })
  await dialog.getByRole('button', { name: 'Next', exact: true }).click()
  await dialog.getByLabel('Google credentials JSON').setInputFiles({ name: 'client.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify({ web: { client_id: 'test.apps.googleusercontent.com', client_secret: 'test-only-secret', redirect_uris: ['https://filament.example.invalid/google/callback'] } })) })
  await dialog.getByRole('button', { name: 'Save credentials' }).click()
  await expect(dialog).toHaveCount(0)
  expect(calls).toEqual(['/api/v1/settings/google/setup'])
  expect(await page.evaluate(() => JSON.stringify({ ...localStorage, ...sessionStorage }))).not.toContain('test-only-secret')
})
