...
for (const variant of [
  ...
  test(`Locations, filters, and tare recalculation: ${variant.name}`, async ({ page }) => {
    ...
    await page.goto('/spools?action=weigh')
    await expect(page.getByText('Select a spool to record its full scale weight.')).toBeVisible()
    await page.getByText('SPOOL-001', { exact: true }).click()
    await expect(page.getByRole('dialog', { name: /Weigh SPOOL-001/ })).toBeVisible()
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    await page.goto('/spools?action=load')
    await expect(page.getByText(/active spool changes only after the confirmed physical load/)).toBeVisible()
    await page.getByText('SPOOL-001', { exact: true }).click()
