...
test('template library is usable at desktop and mobile sizes', async ({ page }) => {
  ...
  const derivedWindow = page.getByLabel('Minimum Extrusion Distance Window (mm)', { exact: false })
  await expect(derivedWindow).toHaveValue('0.8')
  await expect(derivedWindow).toHaveAttribute('readonly', '')
  await expect(page.getByLabel('Maximum Retraction Count', { exact: false })).toHaveValue('100')
  await page.getByRole('spinbutton', { name: /^Retraction distance/ }).fill('1.2')
  await expect(derivedWindow).toHaveValue('1.2')
  await derivedWindow.scrollIntoViewIfNeeded()
  await captureEvidence(page, 'template-retraction-v075')
...
test('filament details remember colors and save Cura settings directly', async ({ page }) => {
  ...
  await expect(page.getByLabel('Build plate temperature (°C)', { exact: true })).toHaveCount(1)
  ...
  await page.getByLabel('Build plate temperature (°C)', { exact: true }).fill('61')
  await expect(page.locator('.setting-field--customized')).toHaveCount(3)
  await page.getByLabel('Build plate temperature (°C)', { exact: true }).fill('60')
  await expect(page.locator('.setting-field--customized')).toHaveCount(2)
...
test('drying guidance is editable in filament settings and displayed in spool details', async ({ page }) => {
  await page.route('**/api/v1/filaments', (route) => route.fulfill({ json: [filament] }))
  ...
  await expect(page.getByRole('spinbutton', { name: /^Filament drying temperature/ })).toBeEditable()
...
test('an empty Cura library can complete the one-time atomic takeover', async ({ page }) => {
  ...
  await expect(dialog.getByText(/No Cura settings will be imported/)).toBeVisible()
...
test('takeover replaces reported Cura sources without importing values', async ({ page }) => {
  const material = {
