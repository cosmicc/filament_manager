...
import { FilamentPlateRatingEditor, PlateCompatibility } from '../components/PlateCompatibility'
...
export default function FilamentDetailPage() {
  ...
  const missingTemplate = (profile: MaterialProfile) => Boolean(templates.data && !templates.data.some((template) => template.active && template.id === profile.base_template_id))
  ...
  return <div className="filament-detail-page">
    ...
    <section className="card product-editor">
      ...
      <PrintActivityDates kind="filament" id={item.id} /><PlateCompatibility filamentId={item.id} editable={canEdit && !item.archived} />
    </section>
      ...
      {!currentProfiles.length ? <EmptyState icon={Plus} title={templates.data?.some((template) => template.active && template.revisions.some((revision) => revision.id === filament.data?.material_template_revision_id)) ? "Print settings not configured" : "Material template unavailable"} description="Add print settings using an active material template for this filament." /> : <div className="profile-scope-grid">{currentProfiles.map((profile) => <article className={`card profile-scope-card${missingTemplate(profile) ? ' profile-scope-card--missing-template' : ''}`} key={profile.id}>
        ...
        {profile.base_template_name ? null : <p className="warning-note">Linked template not available for this filament scope.</p>}
        {missingTemplate(profile) && <p className="warning-note" role="alert">The material template for this scope is unavailable. Choose an active material template.</p>}<footer className="profile-scope-actions">{canEdit ? <><button className="button" onClick={() => { saveProfile.reset(); setEditingProfileId(profile.id) }}><Pencil size={16} /> Edit settings</button><button className="button" onClick={() => setChangingTemplate(profile)}>Change template</button></> : null}<button className="button" onClick={() => setComparisonProfileId(profile.id)}><GitCompareArrows size={16} /> Compare</button><a className="button" href={`/api/v1/profiles/${profile.id}/exports/cura`}><Download size={16} /> Export Cura JSON</a></footer>
      </article>)}</div>}
    </section>
