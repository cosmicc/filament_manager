...
import { CopyPlus, GitCompareArrows, Library, Plus, Upload } from 'lucide-react'
...
export default function TemplatesPage() {
  ...
  const publish = useMutation({
    ...
    onSuccess: async () => {
      setMessage('Template revision published and Cura synchronization queued. Each linked filament must review and confirm this base update separately.')
      await queryClient.invalidateQueries({ queryKey: ['material-templates'] })
    },
    onError: (error: Error) => setMessage(error.message),
  })
  ...
  return <div>
    <PageHeader eyebrow="Reusable inherited bases" title="Material templates" description="Templates synchronize to Cura under the Template brand. Filaments inherit a published base plus explicit overrides and confirm template updates individually." actions={user?.role !== 'viewer' ? <button className="button button--primary" onClick={() => { setRevisionSource(null); setShowEditor(true); setMessage('') }}><Plus size={17} /> Add template</button> : undefined} />
