...
import { PlateRatingEditor, TemplatePlateRatingsButton } from '../components/PlateCompatibility'
...
export default function TemplatesPage() {
  ...
  const beginDeleteFromEditor = () => {
    if (!editSource) return
    deleteTemplate.reset()
    setDeleteConfirmation('')
    setDeleteTarget(editSource)
    setShowEditor(false)
    setEditSource(null)
    save.reset()
  }
  ...
  return <div>
    ...
    <section className="toolbar"><TemplatePlateRatingsButton templates={templates.data ?? []} plates={plates.data ?? []} printers={printers.data ?? []} readOnly={!canEdit} /><CollectionViewSelector label="Templates" value={view} onChange={setView} /><span className="toolbar__summary">{templates.data?.length ?? 0} templates</span></section>
    {loading ? <LoadingState /> : !templates.data?.length ? <EmptyState icon={Library} title="No material templates" description="Add Template PLA, Template PETG, Template ASA, and the other material bases you use." /> : view === 'list' ? <div className="table-card collection-table"><table><thead><tr><th>Template</th><th>Printer / nozzle</th><th>Nozzle / initial bed / bed</th><th>Material flow</th><th>Actions</th></tr></thead><tbody>{templates.data.map((template) => { const latest = template.revisions[0]; const nozzle = nozzles.data?.find((item) => item.id === template.nozzle_id); const printer = printers.data?.find((item) => item.id === template.printer_id); return <tr key={template.id} tabIndex={canEdit ? 0 : undefined} aria-label={canEdit ? `Edit ${template.name}` : undefined} aria-busy={openingTemplateId === template.id || undefined} onClick={canEdit ? () => { void openEditor(template) } : undefined} onKeyDown={canEdit ? (event) => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); void openEditor(template) } } : undefined}><td><strong>{template.name}</strong><small className="table-subtext">{template.material_type}</small></td><td>{printer?.name ?? 'Unknown printer'}<small className="table-subtext">{nozzle ? `${nozzle.nozzle_code} · ${compactNumber(nozzle.diameter_mm, 1)} mm` : 'Unknown nozzle'}</small></td><td>{compactNumber(latest.settings.extruder_temp_c, 0)}° / {compactNumber(latest.settings.initial_bed_temp_c, 0)}° / {compactNumber(latest.settings.bed_temp_c, 0)}°</td><td>{compactNumber(latest.settings.flow_percent, 0)}%</td><td><div className="table-actions">{profiles.data?.length ? <button className="icon-button" onClick={(event) => { event.stopPropagation(); setComparisonTargetKey(`template:${latest.id}`) }} title="Compare settings" aria-label={`Compare ${template.name}`}><GitCompareArrows size={17} /></button> : null}</div></td></tr>})}</tbody></table></div> : <section className={`collection-grid collection-grid--${view}`}>{templates.data.map((template) => {
      ...
      return <article className={`catalog-card catalog-card--template${canEdit ? ' catalog-card--interactive' : ''}${view === 'detailed' ? ' collection-card--detailed' : ''}`} key={template.id} tabIndex={canEdit ? 0 : undefined} role={canEdit ? 'button' : undefined} aria-label={canEdit ? `Edit ${template.name}` : undefined} aria-busy={openingTemplateId === template.id || undefined} onClick={canEdit ? () => { void openEditor(template) } : undefined} onKeyDown={canEdit ? (event) => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); void openEditor(template) } } : undefined}><div className="template-card__identity"><p className="eyebrow">{template.material_type} · {nozzle?.nozzle_code ?? 'Unknown nozzle'}</p><h2>{template.name}</h2><p>{printer?.name ?? 'Unknown printer'}</p></div><dl className="catalog-meta"><div><dt>Physical nozzle</dt><dd>{nozzle ? `${nozzle.nozzle_code} · ${compactNumber(nozzle.diameter_mm, 1)} mm ${nozzle.material}` : `${compactNumber(template.nozzle_diameter_mm, 1)} mm · unavailable`}</dd></div><div><dt>Drying temperature</dt><dd>{latest.settings.drying_temp_c == null ? 'Not set' : `${compactNumber(latest.settings.drying_temp_c, 0)} °C`}</dd></div><div><dt>Nozzle / initial bed / bed</dt><dd>{compactNumber(latest.settings.extruder_temp_c, 0)}° / {compactNumber(latest.settings.initial_bed_temp_c, 0)}° / {compactNumber(latest.settings.bed_temp_c, 0)}°</dd></div><div><dt>Drying time</dt><dd>{latest.settings.drying_time_hours == null ? 'Not set' : `${latest.settings.drying_time_hours} hours`}</dd></div>{view === 'detailed' ? <><div><dt>Filament diameter</dt><dd>{compactNumber(template.filament_diameter_mm, 2)} mm</dd></div><div><dt>Status</dt><dd>{template.active ? 'Active' : 'Inactive'}</dd></div></> : null}</dl><div className="template-card__actions">{profiles.data?.length ? <button className="button" onClick={(event) => { event.stopPropagation(); setComparisonTargetKey(`template:${latest.id}`) }}><GitCompareArrows size={16} /> Compare settings</button> : null}</div></article>
    })}</section>}
