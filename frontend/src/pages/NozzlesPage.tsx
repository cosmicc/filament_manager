...
import { History, Pencil, Plus, Save, Unplug, Wrench } from 'lucide-react'
...
export default function NozzlesPage() {
  ...
  return <div>
    ...
    {nozzles.isLoading ? <LoadingState /> : !nozzles.data?.length ? <EmptyState icon={Wrench} title="No physical nozzles recorded" description="Add the nozzle currently installed in the printer, then record its installation to begin exact print-use tracking." action={canEdit ? <button className="button button--primary" onClick={() => setEditing('new')}><Plus size={17} /> Add first nozzle</button> : undefined} /> : <section className="integration-grid nozzle-grid">
      {nozzles.data.map((nozzle) => {
        ...
        return <article className="integration-card nozzle-card" key={nozzle.id}>
          ...
          <div className="detail-actions">
            <button className="button" onClick={() => setSelected(nozzle)}><History size={16} /> History</button>
            {canEdit ? <button className="button" onClick={() => setEditing(nozzle)}><Pencil size={16} /> Edit</button> : null}
          </div>
