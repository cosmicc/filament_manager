...
function NozzleEditor({ nozzle, pending, error, onClose, onSave }: {
  nozzle: Nozzle | null
  pending: boolean
  error: string
  onClose: () => void
...
export default function NozzlesPage() {
  ...
  const [printerSelection, setPrinterSelection] = useState<Record<string, string>>({})
  ...
  const renderDetailedNozzle = (nozzle: Nozzle) => {
    ...
    return <article className={`integration-card nozzle-card nozzle-card--detailed${nozzle.installed_printer_id ? ' nozzle-card--installed' : ''}`} key={nozzle.id}>
      ...
      {canEdit && nozzle.status !== 'retired' ? <div className="inline-action-group">
        {nozzle.installed_printer_id ? <button className="button" disabled={remove.isPending} onClick={() => remove.mutate(nozzle)}><Unplug size={16} /> Record removal</button> : <><label>Install on<select aria-label={`Printer for ${nozzle.nozzle_code}`} value={selectedPrinterId} onChange={(event) => setPrinterSelection((current) => ({ ...current, [nozzle.id]: event.target.value }))}>{printers.data?.map((printer) => <option key={printer.id} value={printer.id}>{printer.name}</option>)}</select></label><button className="button button--primary" disabled={!selectedPrinterId || install.isPending} onClick={() => install.mutate({ nozzle, printerId: selectedPrinterId })}>Record installation</button></>}
      </div> : null}
  ...
  return <div>
    ...
    {nozzles.isLoading ? <LoadingState /> : !nozzles.data?.length ? <EmptyState icon={Wrench} title="No physical nozzles recorded" description="Add the nozzle currently installed in the printer, then record its installation to begin exact print-use tracking." action={canEdit ? <button className="button button--primary" onClick={() => setEditing('new')}><Plus size={17} /> Add first nozzle</button> : undefined} /> : !visibleNozzles.length ? <EmptyState icon={Search} title="No nozzles match" description="Adjust the search to see another physical nozzle." /> : view === 'list' ? <div className="table-card collection-table"><table><thead><tr><th>Nozzle</th><th>Product</th><th>Status</th><th>Installed on</th><th>Prints</th><th>Filament</th><th aria-label="Actions" /></tr></thead><tbody>{visibleNozzles.map((nozzle) => { const installedPrinter = printers.data?.find((printer) => printer.id === nozzle.installed_printer_id); return <tr key={nozzle.id} tabIndex={0} onClick={() => setDetailsNozzle(nozzle)} onKeyDown={(event) => (event.key === 'Enter' || event.key === ' ') && setDetailsNozzle(nozzle)}><td><strong>{nozzle.nozzle_code}</strong><small className="table-subtext">{compactNumber(nozzle.diameter_mm, 1)} mm {nozzle.material}</small></td><td>{[nozzle.manufacturer, nozzle.product_name, nozzle.coating].filter(Boolean).join(' · ') || 'Not recorded'}</td><td><StatusPill status={nozzle.status} /></td><td>{installedPrinter?.name ?? 'Not installed'}</td><td>{nozzle.completed_print_count.toLocaleString()}</td><td>{Number(nozzle.completed_filament_weight_g).toLocaleString(undefined, { maximumFractionDigits: 1 })} g</td><td><button className="button" onClick={(event) => { event.stopPropagation(); setDetailsNozzle(nozzle) }}>Open details</button></td></tr> })}</tbody></table></div> : view === 'detailed' ? <section className="collection-grid collection-grid--detailed">{visibleNozzles.map(renderDetailedNozzle)}</section> : <section className="collection-grid collection-grid--cards">{visibleNozzles.map((nozzle) => { const installedPrinter = printers.data?.find((printer) => printer.id === nozzle.installed_printer_id); return <button className={`collection-card collection-card--button${nozzle.installed_printer_id ? ' nozzle-card--installed' : ''}`} key={nozzle.id} onClick={() => setDetailsNozzle(nozzle)}><header className="collection-card__header"><span className="integration-card__icon"><Wrench size={23} /></span><StatusPill status={nozzle.status} /></header><div className="collection-card__body"><p className="eyebrow">{nozzle.nozzle_code}</p><h2>{compactNumber(nozzle.diameter_mm, 1)} mm {nozzle.material}</h2><p>{[nozzle.manufacturer, nozzle.product_name].filter(Boolean).join(' · ') || 'No product details recorded'}</p></div><dl className="catalog-meta"><div><dt>Installed on</dt><dd>{installedPrinter?.name ?? 'Not installed'}</dd></div><div><dt>Completed prints</dt><dd>{nozzle.completed_print_count.toLocaleString()}</dd></div><div><dt>Recorded filament</dt><dd>{Number(nozzle.completed_filament_weight_g).toLocaleString(undefined, { maximumFractionDigits: 1 })} g</dd></div><div><dt>Coating</dt><dd>{nozzle.coating ?? 'None recorded'}</dd></div></dl><span className="collection-card__link">Open details and actions</span></button>})}</section>}
    ...
    {editing ? <NozzleEditor nozzle={editing === 'new' ? null : editing} pending={save.isPending} error={save.error?.message ?? ''} onClose={() => setEditing(null)} onSave={(values) => save.mutate({ nozzle: editing === 'new' ? null : editing, values })} /> : null}
