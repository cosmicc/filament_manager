...
export default function CalibrationPage() {
  ...
  const applySettings = useMutation({ mutationFn: () => apiFetch(`/calibrations/${selected?.id}/apply-profile-settings`, { method: 'POST' }), onSuccess: async () => { await Promise.all([client.invalidateQueries({ queryKey: ['calibrations'] }), client.invalidateQueries({ queryKey: ['profiles'] })]) }, onError: (caught) => setError(caught instanceof Error ? caught.message : 'Could not apply calibration settings') })
