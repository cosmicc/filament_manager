...
import { AlertTriangle, CheckCircle2, ChevronFirst, ChevronLast, ChevronLeft, ChevronRight, Clock3, ExternalLink, FileSearch, History, Rows3, Star } from 'lucide-react'
import { type FormEvent, useMemo, useState } from 'react'
import { apiFetch } from '../api/client'
import type { PrintJob, PrintJobPage, PrintQualityRating } from '../api/types'
...
function printOutcome(job: PrintJob) {
...
export default function PrintHistoryPage() {
  ...
  const query = useQuery({
    queryKey: ['prints', status, page, perPage],
    queryFn: () => apiFetch<PrintJobPage>(`/prints/page?page=${page}&per_page=${perPage}${status ? `&print_status=${status}` : ''}`),
  ...
  return <div>
    ...
    <section className="toolbar print-history-toolbar"><label className="select-field"><History size={17} /><select aria-label="Filter print outcomes" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1) }}><option value="">All outcomes</option><option value="in_progress">In progress</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="cancelled">Cancelled</option><option value="legacy_unknown">Unknown print status</option></select></label><label className="select-field"><Rows3 size={17} /><span>Per page</span><select aria-label="Prints per page" value={perPage} onChange={(event) => { setPerPage(Number(event.target.value) as PrintJobPage['per_page']); setPage(1) }}><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select></label><nav className="print-history-pagination" aria-label="Print history pages"><button className="button button--small" type="button" onClick={() => setPage(1)} disabled={currentPage <= 1}><ChevronFirst size={16} /> First</button><button className="button button--small" type="button" onClick={() => setPage(Math.max(1, currentPage - 1))} disabled={currentPage <= 1}><ChevronLeft size={16} /> Previous</button><button className="button button--small" type="button" onClick={() => setPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage >= totalPages}>Next <ChevronRight size={16} /></button><button className="button button--small" type="button" onClick={() => setPage(totalPages)} disabled={currentPage >= totalPages}>Last <ChevronLast size={16} /></button></nav><span className="toolbar__summary">{query.data ? pageSummary(query.data) : 'Loading print records'}</span></section>
