...
import { PrintHistoryActions } from '../components/PrintHistoryActions'
import { PrintSettingSummary } from '../components/PrintSettingSummary'
...
import { compactNumber, costPerGram, currencyAmount, dateTime, durationText, grams, titleCase } from '../lib/format'

function PrintDate({ value }: { value: string | null }) {
  return value ? <time dateTime={value}>{dateTime(value)}</time> : <>Unavailable</>
}
...
function printOutcome(job: PrintJobSummary) {
  if (job.status === 'legacy_unknown' && job.state_snapshot.stale_closed_at) return 'Interrupted / outcome unknown'
...
function printSettingValue(value: unknown) {
  ...
  // Only scalar numeric literals are presentation-rounded. Cura expressions,
  // names and captured archives remain untouched.
  if (typeof value === 'number' || (typeof value === 'string' && /^-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?$/.test(value))) return compactNumber(value, 2)
...
function PrintDetail({ job, canAssess, onClose }: { job: PrintJob; canAssess: boolean; onClose: () => void }) {
  ...
  return (
    <Modal title={job.filename} description="Immutable print context, inspection evidence, material changes, and outcome history." size="wide" onClose={onClose} footer={<><button className="button" type="button" onClick={() => setShowSettings(true)}><SlidersHorizontal size={16} /> Advanced print settings</button><button className="button button--primary" onClick={onClose}>Close</button></>}>
      ...
      <PrintHistoryActions job={job} canManage={canAssess} />
      {job.assessments.length ? <section><p className="eyebrow">Assessment history</p>{[...job.assessments].sort((left, right) => right.revision - left.revision).map((assessment) => <p key={assessment.id}><strong>{titleCase(assessment.rating)}</strong> · {assessment.defect_tags.map(titleCase).join(', ') || 'No defects'}<br /><small><PrintDate value={assessment.created_at} />{assessment.notes ? ` · ${assessment.notes}` : ''}</small></p>)}</section> : null}
    </Modal>
  )
}
