...
import type { BuildPlate, BuildPlateSurface, MaterialTemplate, Printer } from '../api/types'
import { TemplatePlateRatingsButton } from '../components/PlateCompatibility'
...
function PlateEditorModal({
  ...
  return (
    <Modal
      ...
      description="Keep the physical plate identity, geometry, condition, and notes together."
      onClose={onClose}
      size="wide"
      footer={(
        <>
    ...
    >
      ...
      >
        ...
        <EditorSection title="Condition and use" description="Physical properties, condition, and temperature limits.">
          <div className="form-grid">
  ...
  return (
    <section className={`plate-surface${active ? ' plate-surface--active' : ''}${surface.mesh_available !== true ? ' plate-surface--mesh-missing' : ''}`}>
      ...
      <dl className="definition-list definition-list--compact">
        ...
        <div><dt>Last checked</dt><dd><DateWithAge value={surface.last_mesh_checked_at} /></dd></div>
        <div><dt>Last calibrated</dt><dd>{surface.last_mesh_calibrated_at ? dateTime(surface.last_mesh_calibrated_at) : 'Unknown'}</dd></div>
        <div><dt>Last printed</dt><dd>{surface.last_printed_at ? <DateWithAge value={surface.last_printed_at} /> : 'Unknown'}</dd></div>
        <div><dt>Last activated</dt><dd>{surface.last_activated_at ? <DateWithAge value={surface.last_activated_at} /> : 'Unknown'}</dd></div>
        {surface.last_mesh_observed_at && !surface.last_mesh_calibrated_at ? <div><dt>Calibration detected</dt><dd><DateWithAge value={surface.last_mesh_observed_at} /> · exact time unavailable</dd></div> : null}
      </dl>
      ...
      <div className="build-plate-card__summary">
        ...
        <div className="build-plate-card__identity">
          ...
          <dl className="plate-facts">
            ...
            <div><dt>Last printed</dt><dd>{plate.last_printed_at ? <DateWithAge value={plate.last_printed_at} /> : 'Unknown'}</dd></div>
            <div><dt>Last activated</dt><dd>{plate.last_activated_at ? <DateWithAge value={plate.last_activated_at} /> : 'Unknown'}</dd></div>
          </dl>
