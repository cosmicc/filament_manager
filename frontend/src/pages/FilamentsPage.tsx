      ...
      {currentTemplates.length ? <form id="create-filament" className="editor-form" onSubmit={submit}>
        <EditorSection title="Product identity" description="Choose the starting profile and the labels operators see throughout inventory.">
          <div className="form-grid">
            <label>Starting template<select name="material_template_revision_id" required autoFocus>{currentTemplates.map(({ template, settingsSnapshot }) => <option key={settingsSnapshot.id} value={settingsSnapshot.id}>{template.name} · {template.nozzle_diameter_mm} mm</option>)}</select></label>
