...
import type { Filament, MaterialTemplate, Vendor } from '../api/types'
...
export default function FilamentsPage() {
  ...
  const create = useMutation({
    mutationFn: (form: HTMLFormElement) => {
      ...
      return apiFetch('/filaments', {
        ...
        body: JSON.stringify({
          ...
          color_name: String(data.get('color_name')).trim(),
          color_hex: String(data.get('color_hex') ?? '').replace('#', '').trim() || null,
    ...
    onSuccess: async () => {
      setMessage('Filament product created with a new draft profile copied from its generic template.')
      setShowCreate(false)
      await Promise.all([
  ...
  return <div><PageHeader eyebrow="Product catalog" title="Filaments" description="Controlled material definitions shared by physical spools and calibration profiles." actions={user?.role !== 'viewer' ? <button className="button button--primary" onClick={() => setShowCreate((value) => !value)}><Plus size={17} /> Add filament</button> : undefined} />
    {message && <div className="deployment-note" role="status">{message}</div>}
    {showCreate && <section className="card product-editor"><header className="card__header"><div><p className="eyebrow">Canonical product</p><h2>Add a filament</h2></div><PackageOpen size={21} /></header>{publishedTemplates.length ? <form className="form-grid" onSubmit={submit}>
      <label>Starting template<select name="material_template_revision_id" required>{publishedTemplates.map(({ template, revision }) => <option key={revision.id} value={revision.id}>{template.name} · {template.nozzle_diameter_mm} mm · v{revision.version}</option>)}</select></label>
      ...
      <label>Color name<input name="color_name" maxLength={96} required /></label>
      <label>Color hex<input name="color_hex" type="text" pattern="#?[0-9A-Fa-f]{6}" placeholder="#2F80A5" /></label>
