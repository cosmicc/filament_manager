...
export default function FilamentsPage() {
  ...
  const create = useMutation({
    mutationFn: (form: HTMLFormElement) => {
      ...
      const selected = currentTemplates.find((item) => item.settingsSnapshot.id === data.get('material_template_revision_id'))
      ...
      {currentTemplates.length ? <form id="create-filament" className="editor-form" onSubmit={submit}>
        <EditorSection title="Product identity" description="Choose the starting profile and the labels operators see throughout inventory.">
          <div className="form-grid">
            <label>Starting template<select name="material_template_revision_id" required autoFocus>{currentTemplates.map(({ template, settingsSnapshot }) => <option key={settingsSnapshot.id} value={settingsSnapshot.id}>{template.name} · {compactNumber(template.nozzle_diameter_mm, 1)} mm</option>)}</select></label>
        ...
        <EditorSection title="Physical specifications" description="Record the diameter, density, packaged mass, and material modifiers.">
          <div className="form-grid">
            ...
            <label>Diameter tolerance (mm)<input name="tolerance_mm" type="number" min="0" step="0.01" /></label>
            <label>Density (g/cm³)<input name="density_g_cm3" type="number" min="0.01" step="0.01" defaultValue="1.24" required /></label>
            <label>Nominal net mass (g)<input name="nominal_net_mass_g" type="number" min="1" step="1" defaultValue="1000" required /></label>
