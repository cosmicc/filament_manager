...
export default function WorkstationsPage() {
  ...
  const [reviewAgent, setReviewAgent] = useState<WorkstationAgent | null>(null)
  ...
  return <div>
    ...
    {agents.isLoading || templates.isLoading || printers.isLoading ? <LoadingState /> : !agents.data?.length ? <EmptyState icon={MonitorCog} title="No workstations paired" description="Create a one-time code, install the agent under your normal workstation account, and pair it with Filament Manager." /> : <div className="workstation-grid">{agents.data.map((agent) => {
      const selectedTemplateIds = new Set(agent.cura_materials.map((source) => mappings[`${agent.id}:${source.source_id}`]).filter(Boolean))
      const mappedCount = selectedTemplateIds.size
      return <article className="workstation-card card" key={agent.id}>
        ...
        {!agent.cura_management_enabled ? <section className="cura-preservation" aria-label={`Cura sources reported by ${agent.display_name}`}>
          <div><h3>Map Cura sources to templates</h3><p className="muted">Only tracked literal settings are imported. Each source and template can be used once; unmapped sources are intentionally discarded during takeover.</p></div>
          {!agent.cura_materials.length ? <p className="muted">No existing Cura material or saved print profiles have been reported. You may still complete takeover and import none.</p> : null}
          <div className="cura-material-list">{agent.cura_materials.map((source) => {
            const key = `${agent.id}:${source.source_id}`
            ...
            return <article className="cura-material-item" key={`${source.installation_id}:${source.source_id}`}>
              ...
              <label>Filament Manager template<select aria-label={`Template for ${source.name}`} value={selectedTemplate} disabled={user?.role !== 'administrator'} onChange={(event) => setMappings((current) => ({ ...current, [key]: event.target.value }))}><option value="">Do not import</option>{activeTemplates.map((template) => <option key={template.id} value={template.id} disabled={template.id !== selectedTemplate && selectedTemplateIds.has(template.id)}>{templateLabel(template)}</option>)}</select></label>
            </article>
          })}</div>
          {user?.role === 'administrator' ? <button className="button button--primary" type="button" disabled={!agent.enabled || !agent.cura_installations.length || takeover.isPending} onClick={() => { setMessage(''); setReviewAgent(agent) }}>Review takeover ({mappedCount} mapped)</button> : null}
    ...
    {reviewAgent ? <Modal title="Review Cura takeover" description="This is the one confirmation for all source-to-template choices. The operation is atomic: either every mapping and synchronization state saves, or none do." onClose={() => setReviewAgent(null)} size="wide" footer={<><button className="button" type="button" onClick={() => setReviewAgent(null)}>Back to mappings</button><button className="button button--primary" type="button" disabled={takeover.isPending} onClick={() => takeover.mutate(reviewAgent)}><ShieldCheck size={16} />{takeover.isPending ? 'Completing…' : 'Complete takeover'}</button></>}>
      <EditorSection title="Sources to import" description="Each selected source directly updates the current settings of its mapped template. Linked filament profiles inherit those changes immediately except for explicitly customized values.">
        {reviewAgent.cura_materials.some((source) => mappings[`${reviewAgent.id}:${source.source_id}`]) ? <div className="comparison-table">{reviewAgent.cura_materials.filter((source) => mappings[`${reviewAgent.id}:${source.source_id}`]).map((source) => {
          const template = activeTemplates.find((item) => item.id === mappings[`${reviewAgent.id}:${source.source_id}`])
      ...
      <EditorSection title="Sources to discard" description="Unmapped sources are not added to Filament Manager and are removed from the managed Cura library after backup.">
        <p>{reviewAgent.cura_materials.filter((source) => !mappings[`${reviewAgent.id}:${source.source_id}`]).length} of {reviewAgent.cura_materials.length} reported sources will not be imported.</p>
      </EditorSection>
