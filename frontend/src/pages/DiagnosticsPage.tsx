...
export default function DiagnosticsPage() {
  ...
  return <div>
    ...
    {overview.isLoading ? <LoadingState label="Checking operations" /> : overview.error ? <p className="form-error">{overview.error.message}</p> : <>
      ...
      <section><div className="section-heading"><div><p className="eyebrow">Bounded operational log</p><h2><TriangleAlert size={20} /> Recent errors</h2></div></div>{overview.data?.error_log.length ? <div className="mobile-card-list mobile-card-list--always">{overview.data.error_log.map((entry, index) => <article className="mobile-data-card" key={`${entry.source}-${entry.occurred_at}-${index}`}><div><strong>{entry.summary}</strong><StatusPill status={entry.severity} /></div><span>{entry.source} · {dateTime(entry.occurred_at)}</span><small>{entry.detail ?? 'No additional detail retained.'}</small></article>)}</div> : <EmptyState icon={Activity} title="No recent operational errors" description="Failed projections, Cura deployments, and active error notifications appear here without external response bodies." />}</section>
    </>}
