...
export default function DiagnosticsPage() {
  ...
  const [errorDays, setErrorDays] = useState<1 | 7 | 30>(1)
  const overview = useQuery({ queryKey: ['diagnostics', errorDays], queryFn: () => apiFetch<DiagnosticOverview>(`/diagnostics?error_days=${errorDays}`), refetchInterval: 30_000 })
  ...
  return <div>
    ...
    <section className="diagnostic-actions card">
      ...
      {latestRun ? <div className="validation-result">
        ...
        {latestRun.results.checks?.length ? <><p className="muted">These recorded checks reflect conditions when validation ran. Live diagnostics below show the current state.</p><CheckGrid checks={latestRun.results.checks} /></> : null}
    ...
    {overview.isLoading ? <LoadingState label="Checking operations" /> : overview.error ? <p className="form-error">{overview.error.message}</p> : <>
      ...
      <section><div className="section-heading"><div><p className="eyebrow">Bounded operational log</p><h2><TriangleAlert size={20} /> Recent errors</h2><p>This history remains available after recovery. Entries marked History do not change the live health checks above.</p></div><div className="detail-actions"><label className="select-field"><span>Show</span><select aria-label="Recent error period" value={errorDays} onChange={(event) => setErrorDays(Number(event.target.value) as 1 | 7 | 30)}><option value={1}>Last day</option><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option></select></label><a className="button" href={`/api/v1/diagnostics/log.txt?error_days=${errorDays}`}><Download size={16} /> Download log</a></div></div>{overview.data?.error_log.length ? <div className="mobile-card-list mobile-card-list--always">{overview.data.error_log.map((entry, index) => <article className="mobile-data-card" key={`${entry.source}-${entry.occurred_at}-${index}`}><div><strong>{entry.summary}</strong><StatusPill status={entry.current ? entry.severity : 'disabled'} label={entry.current ? titleCase(entry.severity) : 'History'} /></div><span>{entry.source} · {dateTime(entry.occurred_at)}</span><small>{entry.detail ?? 'No additional detail retained.'}</small></article>)}</div> : <EmptyState icon={Activity} title="No recent operational errors" description={`No failed projections, Cura synchronization errors, or active error notifications were recorded in the last ${errorDays === 1 ? 'day' : `${errorDays} days`}.`} />}</section>
    </>}
