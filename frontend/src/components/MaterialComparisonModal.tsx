...
function filamentLabel(profile: MaterialProfile, filaments: Filament[]): string {
  ...
  return `${[filament.vendor_name, filament.product_name ?? filament.material_type].filter(Boolean).join(' ')} · ${filament.color_name} · profile v${profile.version}`
}
...
export function MaterialComparisonModal({ profiles, templates, printers, filaments, plates, catalog, initialProfileId, initialTargetKey, onClose }: {
  ...
  const targets = useMemo<ComparisonTarget[]>(() => [
    ...
    ...templates.flatMap((template) => template.revisions.map((revision) => ({ key: `template:${revision.id}`, type: 'template' as const, sourceId: revision.id, label: `${template.name} · template v${revision.version}`, optionLabel: `${template.name} · template v${revision.version} · ${scopeLabel({ printerId: template.printer_id, nozzleDiameterMm: template.nozzle_diameter_mm }, printers)}`, printerId: template.printer_id, nozzleDiameterMm: template.nozzle_diameter_mm, settings: revision.settings }))),
  ...
  return <Modal title="Compare material settings" description="Select two to four saved profiles or template revisions. The first selection is the baseline and only semantic differences are shown." onClose={onClose} size="wide" footer={<button className="button" onClick={onClose}>Close comparison</button>}>
    <div className="comparison-layout">
      <EditorSection title="Comparison set" description="Choose up to four immutable versions. Cross-printer and cross-nozzle comparisons remain available with a clear warning."><div className="comparison-target-grid">{targets.map((target) => <label className="check-row" key={target.key}><input type="checkbox" checked={selectedKeys.includes(target.key)} disabled={!selectedKeys.includes(target.key) && selectedKeys.length >= 4} onChange={() => toggleTarget(target.key)} /><span><strong>{target.label}</strong><small>{target.optionLabel}</small></span></label>)}</div><small className="field-help">{selected.length}/4 selected · first selected is the baseline</small></EditorSection>
      ...
      <section className="comparison-results"><header><div><p className="eyebrow">Difference-only view</p><h3>{differenceRows.length} setting {differenceRows.length === 1 ? 'difference' : 'differences'}</h3></div></header>{differenceRows.length ? <div className="comparison-table comparison-table--multi" style={{ '--comparison-columns': selected.length } as CSSProperties}><div className="comparison-row comparison-row--multi comparison-row--header"><span>Setting</span>{selected.map((target) => <span key={target.key}>{target.label}</span>)}</div>{differenceRows.map((row) => <div className="comparison-row comparison-row--multi" key={row.key}><div className="comparison-setting"><strong>{row.label}</strong><small>{row.key}</small></div>{selected.map((target) => <div className="comparison-value" key={target.key}><span>{target.label}</span><strong>{row.values.get(target.key) ?? 'Same as baseline'}</strong></div>)}</div>)}</div> : <div className="comparison-empty"><CheckCircle2 size={22} /><div><strong>No setting differences</strong><p>The selected versions store equivalent material settings.</p></div></div>}</section>
      <section className="comparison-results"><header><div><p className="eyebrow">Recorded outcomes</p><h3>Success rate by version</h3></div></header><div className="comparison-stat-grid">{selected.map((target) => { const stats = target.type === 'profile' ? statistics.data?.[target.sourceId] : undefined; return <article key={target.key}><strong>{target.label}</strong>{target.type === 'template' ? <span>N/A</span> : <span>{stats?.success_rate_percent ? `${stats.success_rate_percent}%` : 'No ratings'}</span>}<small>{target.type === 'template' ? 'Templates are not printed directly.' : stats?.low_sample ? `Low sample · ${stats.rated_prints} rated` : `${stats?.rated_prints ?? 0} rated prints`}</small></article> })}</div></section>
    </div>
  </Modal>
}
