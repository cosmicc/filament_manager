...
- `material_templates` are mutable printer/nozzle-scoped identities; `material_template_revisions` are complete immutable settings snapshots after publication. Every material-profile revision links to one published base revision, stores a bounded sparse `setting_overrides` map, and caches the complete resolved snapshot in typed columns plus `cura_extensions`. Preserve that snapshot for history, and create a separate draft for each explicitly confirmed per-filament base update.
- `cura_managed_edit_receipts` makes workstation-reported edits to known managed GUIDs idempotent. It may reference a created draft profile or template revision; unknown/new Cura GUIDs never create canonical records.
