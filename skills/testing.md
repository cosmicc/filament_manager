...
- Moonraker tests cover the supported active Spoolman ID endpoint, persistent physical-spool macro state, bounded catalog serialization, live manual selection, guarded direct Spoolman target requests plus physical-ID restoration, 15-second state scheduling, 5-minute printer-information scheduling, and safe partial progress when one state request fails.
