...
- Cura material preservation tests cover reported workstation data, one scoped family template plus product-profile imports, cross-resource source provenance uniqueness, duplicate rejection, publication, and the server-side takeover guard.
...
- Moonraker tests cover the supported active Spoolman ID endpoint, persistent physical-spool macro state, separately bounded strict-print and safe-manual catalogs, draft/template manual temperatures, live manual selection, guarded direct Spoolman target requests plus physical-ID restoration, 15-second state scheduling, 5-minute printer-information scheduling, and safe partial progress when one state request fails.
