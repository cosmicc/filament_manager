...
- Cura takeover tests cover reported material files and saved print profiles including zero-literal/expression-only sources, the two-stage mapping dialog and its back action, optional source-to-existing-template mappings, source/template uniqueness, ignored sources, atomic confirmation, direct template saves, linked-profile inheritance, repeated-takeover rejection, and the server-side takeover guard.
