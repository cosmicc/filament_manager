...
- Account/notification tests cover forced temporary-password route gating, reset/deactivation session revocation, last-Administrator protection, per-user reads, recurrence becoming unread, and deduplicated category resolution.
- Build-plate tests cover immutable maintenance events, configurable day/print due thresholds, cleaning and per-side mesh actions, and physical mesh clearing before canonical context is cleared.
...
- Diagnostics regression tests cover the current Alembic head, authenticated text-download headers and bounded content, SQL/URL sanitization, stale-agent guidance, and safe aggregate worker failure summaries.
...
- Workstation-agent tests use isolated temporary Cura roots and cover machine discovery, hardened approved-key material import, bounded saved print-profile global/first-extruder merging, expression omission, complete desired-library XML/plugin rendering including Klipper plugin keys, bundled-choice filtering, exclusive user-material replacement, root/symlink rejection, idempotent apply, drift detection, backup, and rollback.
...
- Cura takeover tests cover reported material files and saved print profiles, optional source-to-existing-template mappings, source/template uniqueness, ignored sources, atomic confirmation, direct template saves, linked-profile inheritance, repeated-takeover rejection, and the server-side takeover guard.
- Profile-inheritance tests cover decimal-equivalent sparse overrides, extension removal, resolved snapshots, per-setting reset, preserved override ownership across direct template saves, and immediate propagation to every linked filament.
...
- Frontend tests and rendered Playwright checks cover the shared grouped editor, removal of fold-down record editors, automatic freshness copy, active-spool indicators, the **Load spool** request and pending Fluidd guidance, version presentation and release comparison, Settings-only theme selection, simplified logout/collapse controls, keyboard focus containment, and desktop/mobile Build Plates and Printers layouts.
...
- Spoolman contract tests cover managed-field provisioning, JSON-encoded custom values, pagination, duplicate-safe UUID discovery, full canonical convergence, non-destructive usage ordering, canonical three-decimal mass normalization, and stale-job recovery. Validate contract-sensitive changes against the pinned real Spoolman image in addition to mocks.
