...
async def test_version_status_includes_prereleases_and_caches_result(
    ...
    assert first.running_version == "0.5.1"
    assert first.latest_version == "0.5.1"
    assert first.status == "current"
    assert first.release_url == "https://github.com/cosmicc/filament_manager/releases/tag/v0.5.1"
    assert second == first
