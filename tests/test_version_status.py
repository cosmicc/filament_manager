...
async def test_version_status_includes_prereleases_and_caches_result(
    ...
    route = respx.get(version_service.GITHUB_RELEASES_URL).mock(
        return_value=httpx.Response(
            ...
            json=[
                ...
                {"tag_name": "v0.6.5", "draft": False, "prerelease": True},
    ...
    assert first.running_version == "0.6.5"
    assert first.latest_version == "0.6.5"
    assert first.status == "current"
    assert first.release_url == "https://github.com/cosmicc/filament_manager/releases/tag/v0.6.5"
    assert second == first
