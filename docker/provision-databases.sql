...
SELECT format('CREATE ROLE %I LOGIN', 'filament_user')
WHERE NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'filament_user') \gexec
ALTER ROLE filament_user PASSWORD :'filament_manager_password';
...
SELECT format('CREATE DATABASE %I OWNER %I', 'filament_manager', 'filament_user')
