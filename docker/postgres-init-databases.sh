...
SELECT format('CREATE ROLE filament_user LOGIN PASSWORD %L CONNECTION LIMIT 30', :'filament_manager_password')
WHERE NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'filament_user')\gexec
...
SELECT 'CREATE DATABASE filament_manager OWNER filament_user ENCODING ''UTF8'''
...
GRANT CONNECT, TEMPORARY ON DATABASE filament_manager TO filament_user;
GRANT CONNECT, TEMPORARY ON DATABASE spoolman TO spoolman_user;

\connect filament_manager
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
GRANT USAGE, CREATE ON SCHEMA public TO filament_user;
