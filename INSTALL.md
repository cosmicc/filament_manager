...
- Copy `integrations/klipper/filament-manager-macros.cfg` to the Klipper configuration directory and include it **last**, after the files that define the existing `START_PRINT`, `END_PRINT`, `CANCEL_PRINT`, `LOAD_FILAMENT`, `UNLOAD_FILAMENT`, and `PURGE_FILAMENT` macros. Remove or disable any other `M600` definition: Filament Manager owns that complete workflow and does not require a pre-existing command. The integration wraps the other existing routines; it does not replace their physical movement or completed-print behavior.
...
- Allow the 0.5.7 workstation agent to install renderer revision 13 while Cura is closed, then start Cura once. For a selected managed product material, its plugin supplies this start boundary at slice time:

```gcode
FILAMENT_MANAGER_START_PRINT MATERIAL_GUID={material_guid} BED_TEMP={material_bed_temperature_layer_0} EXTRUDER_TEMP={material_print_temperature_layer_0} CHAMBER_TEMP={build_volume_temperature}
```
