...
Before takeover, open **Cura workstations** and wait for the agent to report unmanaged material import sources and user-saved custom print profiles. After takeover, the page reports the synchronized managed-material count instead; zero user-saved custom print profiles is normal when Cura has no custom quality profiles. If an expected source count is unexpectedly zero, upgrade or restart the agent, keep Cura closed, and wait for its next check-in. Version 0.6.7 continues to use Cura deployment schema 3, and every paired workstation should run the 0.6.7 agent for synchronized version reporting and the current material, edit, quality-profile isolation, print-boundary, nozzle-reconciliation, and recovery behavior. Select **Map Cura profiles**, map any source you want to keep to an existing template, and leave all others as **Do not import**. A recommended `Template ASA` is seeded for the configured printer/nozzle when no ASA template already exists. Use **Review takeover**, verify the mappings and ignored count, then select **Complete takeover**. Routine agent check-ins do not invalidate the dialog; an actual source-content change requires a fresh review. A genuinely clean installation can complete with zero mappings. Cura shows and favorites bases as `Template PLA`, `Template PETG`, and so on under brand `Template`; manufacturer-less products appear under `Unknown`. Every generated product label appends its meaningful filler once, so Cura distinguishes names such as `SUNLU Black Carbon Fiber` and `SUNLU Black`; missing, blank, `None`, and `No filler` values remain filler-free, as do all template names. Every generated material description lists its filler and finish on separate labeled lines, using `None` when a field is empty. Managed product and template container identities remain stable across later settings revisions. Renderer revision 18 remains current because 0.6.7 does not change generated workstation files. During its first deployment while Cura is closed, the agent backs up and installs the managed material library and saved machine scripts. Later template or product saves queue the complete library automatically. Close Cura when synchronization is pending; the agent retries without manual requeueing. A local automatic-backup failure appears only under Cura recovery with bounded path-free guidance; failed named requests remain in Recovery points history and do not make an otherwise connected agent appear unhealthy or block other work.
