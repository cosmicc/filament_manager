...
Installing a different physical nozzle queues a closed-Cura machine update for every managed workstation. The agent selects only one existing matching Cura nozzle variant for the exact printer, backs up the machine instance, and then re-synchronizes the material library. If Cura has no single matching variant, the operation fails safely and reports that action instead of manufacturing an unsupported machine profile.
