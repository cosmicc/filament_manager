...
Use the application **Diagnostics** page as the consolidated status surface for running/latest version, connections, synchronization freshness, worker heartbeats, actionable projection-queue summaries, bounded recent errors, recovery validation, safe projection rebuilding, and canonical database backups. Never show the individual Projection operations / Recent jobs list. **Download log** saves the same current bounded overview as a non-cacheable sanitized text file; it is an operational summary, not raw application or database output. The version lookup uses the fixed public GitHub releases API, includes non-draft testing releases, is cached for 15 minutes, and never returns an upstream response body. Integrations remains a configuration/ownership guide rather than a duplicate live-status dashboard.
