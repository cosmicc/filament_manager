...
Use the application **Diagnostics** page as the consolidated status surface for running/latest version, connections, synchronization freshness, worker heartbeats, actionable projection-queue summaries, bounded recent errors, recovery validation, safe projection rebuilding, and canonical database backups. Do not provide a separate Integrations browser page. Never show the individual Projection operations / Recent jobs list. Recent errors and **Download log** use the same exact one-day, seven-day, or thirty-day cutoff, defaulting to one day; the download is a non-cacheable sanitized operational summary, not raw application or database output. The version lookup uses the fixed public GitHub releases API, includes non-draft testing releases, is cached for 15 minutes, and never returns an upstream response body.

Canonical mutations normally begin Spoolman projection within one worker polling cycle. The default one-minute full sweep imports usage before repairing every canonical vendor, filament, and spool. Normalize remote remaining weight to canonical three-decimal gram precision before comparing or recording usage so additional source precision cannot generate a duplicate event. Terminal completed, failed, and cancelled print capture also aggregates Moonraker-reported actual segment use into one idempotent event per exact spool; it preserves a lower already-imported Spoolman boundary and never substitutes predicted use. The 10-second Moonraker state job treats the initialized macro's last completed physical boundary as authority, turns valid direct non-null selections in safe selection phases into guarded Fluidd targets, restores the physical active ID until confirmation, refreshes the bounded Cura/manual-load spool catalog, and aligns plate state; the 5-minute printer-information job refreshes sanitized discovered fields. Both jobs defer during an active print, while one combined 10-second live print/preflight object query remains active. A failed live print capture must reload transaction-expired printer state before continuing supported history reconciliation. A successful recurring job marks older dead rows of its type as superseded, retaining history without reporting recovered failures as actionable queue debt. Moonraker state failures identify the bounded failing sub-operation. Confirm the Diagnostics Spoolman check reports both API and managed-field readiness, and confirm recent `spoolman.reconcile.full`, `moonraker.state.reconcile`, and `moonraker.printer_info.reconcile` jobs complete. Structured web and worker logs include safe request, scheduler, job, and synchronization diagnostics with correlation IDs but never credentials or external response bodies.
...
- canonical application ZIP status, schedule, retention, and at least one downloaded off-host copy
...
The worker creates the first due logical snapshot and then follows the Diagnostics interval. The default is 24 hours with ten automatic snapshots. Defer dump creation while a fresh print is in progress; if an interrupted printer leaves a stale canonical in-progress row, issue only the minimal live-state query and allow the backup only after every configured printer reports terminal state. Fail closed when that confirmation is unavailable. After an automatic failure retry after a persisted exponential delay beginning at fifteen minutes and capped at six hours. The runtime PostgreSQL 18 client supports the application's PostgreSQL 18 and older supported-server backup contract. Backups are private compressed ZIP files in the shared application data volume, and storage/tool failures expose only bounded actionable guidance. Administrators may create/download/import archives, but only a typed-confirmed exact archive can become the pending restore. Stop web and worker before scaling the zero-replica `database-restore` service to one. Confirm its successful one-shot completion, scale it back to zero, and then restart normal services. Never run it concurrently with web or worker, and keep Spoolman's independent platform restore separate.
