...
Live display names are derived from material type, color, meaningful filler, and meaningful finish joined by ` · `. Blank, `None`, `Standard`, `Not specified`, `No filler`, and `No finish` modifiers are omitted case-insensitively. The legacy `product_name` column remains for backward compatibility; it does not control live labels and is not rewritten during upgrades. Immutable historical snapshots remain unchanged.
