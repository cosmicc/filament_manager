...
The same file provides bounded plate behavior: `SELECT_BUILD_PLATE` accepts only exact `P<number>` Side A or `P<number>b` Side B values before passing the value to `BED_MESH_PROFILE LOAD`. With no parameter, it restores the locally persisted side without prompting. `CHOOSE=1` enumerates `printer.bed_mesh.profiles` live and displays only those valid names as Fluidd buttons. See [Build plate setup](../BUILD_PLATE_SETUP.md) for automatic calibration and startup behavior.

## Cura print preflight

While Cura is closed, the workstation agent safely backs up and overwrites the matched printer's saved start G-code with `FILAMENT_MANAGER_START_PRINT MATERIAL_GUID={material_guid, 0} BED_TEMP={material_bed_temperature_layer_0, 0} REGULAR_BED_TEMP={material_bed_temperature, 0} EXTRUDER_TEMP={material_print_temperature_layer_0, 0} CHAMBER_TEMP={build_volume_temperature}` and its saved end G-code with `END_PRINT`. The explicit extruder-zero placeholders ensure Cura obtains the material GUID and per-extruder temperatures from the material-bearing extruder stack while keeping the resolved initial and regular bed temperatures distinct. The saved boundary is manifest-tracked, restored on a failed transaction, and re-applied after drift, recovery, or nozzle alignment. The worker publishes a bounded mapping from each current managed product-material GUID to eligible projected Spoolman IDs, safe Fluidd labels, and the current exact profile nozzle temperature.
...
The preflight hold uses the virtual-SD `M25`/`M24` pair rather than Klipper's motion-aware `PAUSE`/`RESUME` pair. `M25` makes `virtual_sdcard.is_active` false while the loaded file reports `print_stats.state == "paused"`, so the app retains a dedicated resume latch and must never require `is_active` before issuing `M24`. It releases only when that latch is set, the file remains loaded, the print state is paused, the printer-specific `START_PRINT` is no longer waiting for a build-plate mesh, and `pause_resume.is_paused` is false. An unmet condition retries every half second for no more than thirty seconds, then stops the delayed loop and displays explicit Retry and Cancel actions. A real operator pause therefore remains authoritative. Cancellation recognizes the loaded paused file, cancels the delayed release, clears the latch and attempt counter, and explicitly resets a direct-`M25` virtual-SD file before invoking the printer's original cancellation cleanup; Klipper's base cancellation does not own that hold. The reference reports its version through `FILAMENT_MANAGER_SPOOL_STATE` so the installed file can be verified. If the printer's `START_PRINT` defers to the app-owned `SELECT_BUILD_PLATE`, it exposes its stored temperatures plus `waiting_for_mesh` and `_START_PRINT_CONTINUE`; selecting the plate finishes that printer-owned start routine before the app schedules `M24`.

The application setting `gcode_inspection` defaults to `warn`. In `block` mode the same wrapper pauses before spool selection and waits for `FILAMENT_MANAGER_GCODE_INSPECTION`. Missing exact profile state, an unavailable bounded file inspection, or any supported profile mismatch remains blocked. In `warn` mode the evidence is stored without delaying the spool workflow. Regular and initial-layer build-plate temperatures remain distinct: Moonraker's first-layer value, the managed `BED_TEMP` boundary, and the first bed-heating command may validate only the initial-layer profile value, while a regular-temperature comparison requires the managed boundary's resolved `REGULAR_BED_TEMP`. Missing regular-temperature evidence is skipped rather than inferred from an initial-layer value, and Cura's `SETTING_3 material_bed_temperature` quality-layer value is never accepted as resolved material evidence. The worker reads `/server/files/metadata`, streams `/server/files/gcodes/{filename}` once for its SHA-256 plus bounded header/tail samples, and never evaluates Cura content. For every new exact print it also retains all safely bounded `SETTING_3` values as inert text in separate global and per-extruder scopes, filters credential/location-like data, and records whether any value was filtered or the capture was truncated.

Every ten seconds by default, the worker reads documented `print_stats` and the Filament Manager preflight macro in one combined object query, then converges the live canonical PrintJob. It defers the complete `/server/history/list` import and nonessential state, mesh, catalog, and printer-information synchronization while `print_stats` reports `printing` or `paused`, then resumes them on the first later pass so they do not compete with printer motion. Exact state capture waits until a pending preflight/load finishes so the previously loaded spool is never treated as the new print's starting spool. The supported bounded raw history status is retained so a terminal cancellation, error, Klippy shutdown, Klippy disconnect, or interruption replaces stale in-progress/legacy display state. M600 creates immutable material segments and refreshes their current actual use. Each exact segment snapshot retains the spool's purchase-cost-per-gram and currency basis so current and historical costs never depend on mutable inventory. Historical jobs with insufficient canonical context are retained as explicitly unresolved instead of guessed.
