...
Templates also provide independently remembered List, Cards, and Detailed views. Each template can be downloaded as a bounded, versioned JSON document without environment-specific database IDs. Import requires an explicit choice between creating a selected printer/nozzle/material scope and overwriting only the settings of one selected existing template; overwrite requires confirmation and preserves the target identity and scope. Mouse-wheel gestures over focused numeric inputs must scroll the page or modal without stepping the input, while keyboard arrow keys and typed entry remain supported. Operator-defined multicolor requests remain limited to three samples, but response contracts accept Rainbow's complete fixed six-sample palette. Filament create and edit routes validate their response representation before commit so a response mismatch cannot persist an unreadable catalog mutation.
