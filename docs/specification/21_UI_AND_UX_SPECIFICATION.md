...
The Templates page refreshes the selected template immediately before opening and directly edits complete material settings named `Template <material type>` with brand `Template`. Server or native validation strongly highlights the exact rejected control, renders its sanitized reason, centers it, and moves keyboard focus there; a genuine update after opening remains an explicit optimistic-concurrency conflict. Print settings and Templates expose one shared read-only comparator: choose two to four current profiles/templates, with the first as the baseline. The result contains only settings whose canonical values differ, treats equivalent decimal representations as equal, displays all scopes, and shows exact-profile outcome rates; template statistics are N/A and samples below five are labeled low. Any printer or nozzle pairing is allowed, but a prominent warning identifies each mismatched scope dimension. The Filaments Catalog requires a current template for the first scope, renders arbitrary shared solid colors, product-specific one/two/three-sample multicolor palettes, and fixed Rainbow color, and links each product to its detail. Opening the color control shows known colors again while allowing a new typed name. A filament card uses three identity lines: blue vendor; bold material type, color name, and optional display name; then smaller filler and finish separated by `/`. Every palette mode uses the same physical spool silhouette. Color remains editable until retained use exists. Filament detail separates physical product editing from responsive print-settings cards and may show multiple current exact filament/printer/nozzle scopes. Each scope card shows printer, nozzle, linked template, inherited/customized state, key settings, and exact Edit, Compare, and Cura JSON export actions. Operators may add an unoccupied scope from any compatible active same-material template; product edits never silently rebase print settings. Profile details show the linked template, inherited/customized count, and template values beside each profile-editable setting. Settings use Cura-like named groups without an advanced catch-all; Retraction Retract Speed and Retraction Prime Speed are independent. Print speeds and cooling other than Initial Fan Speed are template-only. Initial Fan Speed, pressure advance, and ironing are editable on both templates and profiles. Smooth time and the seven approved acceleration values remain template-only, and the two hidden acceleration toggles are always enabled in Cura. Flow has one canonical visible control, and the only temperature controls are Printing Temperature, Build Volume Temperature, and Build Plate Temperature. An edit that differs from its inherited value highlights immediately. Saving a template immediately updates every current exact scope that still inherits it except explicitly customized permitted keys; saving any known profile/template in Filament Manager or Cura queues synchronization automatically. Profile Cura settings download as JSON attachments. The Spools page shows completed prints, creates physical spools from purchased filament-only weight plus optional full scale weight with an inferred-tare preview, displays purchase cost and calculated cost per gram, identifies the associated printer, provides full setup correction/delete-or-archive actions, lets Operators edit or clear free-text bucket locations, and offers a physical **Unload** action that clears Spoolman only after completed motion. Its **Load spool** action reports that Fluidd will request physical confirmation, automatically follows the unload/insert/load workflow when another spool is present, and keeps the current active spool visible until that load finishes. New materials can be added only in Filament Manager.
