...
Filaments contains a secondary navigation with **Catalog** at `/filaments` and **Print settings** at `/filaments/settings`, placed immediately before the page's primary action so the page title remains at the top. The latter is the current filament/printer/nozzle settings matrix; Profiles is not a top-level destination. The legacy browser `/profiles` path redirects to Print settings, while backend profile and integration API paths remain unchanged. There is no Integrations browser page; Diagnostics and the workflow pages are the browser surfaces for those capabilities.
...
The Dashboard does not duplicate Diagnostics. Its title has no descriptive subtitle. The full-width live printer status card is the first card, with the three inventory value cards directly beneath it before the remaining printing-context cards. While printing, its state/progress region receives more desktop width and the nozzle, bed, and chamber temperatures become three compact cards; ordinary and narrow layouts retain the readable shared arrangement. Quick Actions spans the available width in a horizontal layout and includes weighing a spool, printing labels, loading/selecting filament, and adding filament. The live printer state pill is substantially larger than ordinary status pills and remains comfortably contained by the existing printer-card header at desktop and mobile widths without increasing the card dimensions.
...
Compact responsive summaries begin with physical P1-P5 and naturally order later discovered plates, using multiple readable columns when the viewport allows. The page provides an **Add Build Plate** action that transactionally creates the next P-number, a matching display name, and one mesh-unavailable Side A record. Each physical card uses its sanitized uploaded picture in the plate icon when present and shows its description, manufacturer/product, shape/dimensions, magnetic/flexible state, condition/status, preferred materials, temperature limit, cleaning state, configurable day/print reminder thresholds, and nested Side A/Side B panels with exact mesh, surface material, smooth/textured finish, mesh availability/check/calibration time, notes, and completed-print count. A side without its exact Klipper/Moonraker/Fluidd heatmap profile receives a distinct semantic warning color and explicit **No matching Klipper heatmap profile** text plus the required exact profile name. Physical, picture, cleaning, side-selection, and mesh controls wrap at their intrinsic width on desktop and expand to full-width 44-pixel touch actions only on narrow mobile layouts. The editor offers only rectangular and round shapes, shows width/depth exclusively for rectangular geometry and diameter exclusively for round geometry, and clears irrelevant hidden values on save. Cleaning and mesh actions append maintenance events. The active side is visually explicit and may be cleared only through the physical Moonraker workflow. Operators may add the one Side B record; it remains unavailable until its exact mesh is discovered. Server-side state synchronizes automatically every 10 seconds while no print is active.

Before authoritative takeover, Cura Workstations offers an explicit review and confirmation of replacing the reported Cura library after backup. There are no import mappings. The reviewed content-hashed source set must still match at confirmation. Each managed workstation offers Push app settings with pending/succeeded/failed feedback, offline guidance, and a reminder to close Cura before deployment.
...
Nozzles starts with a printer selector like Build Plates and shows only that printer's inventory. Each record shows its permanent printer, editable printer-scoped physical code, diameter, construction material, lifecycle state, installation state, completed prints, total filament use, and append-only install/remove history. The currently installed nozzle card has a subtle semantic highlight in addition to its status label. Installing a nozzle replaces no record silently, rejects a different printer, and one printer has at most one installed nozzle.

Diagnostics shows the running application version and cached newest non-draft GitHub release, then groups connection, synchronization, worker, and operational information; exact per-installation Cura material-setting verification; actionable queue summaries without an individual Projection operations / Recent jobs table; one newest sanitized cause per failing job type; bounded recent errors with exact **Last day**, **Last 7 days**, and **Last 30 days** choices defaulted to the last day; a matching **Download log** text action; persisted read-only validation results; and an Administrator-only safe projection rebuild. At the bottom, a canonical database-backup card shows a clearly grouped enable/interval/retention schedule, last success or bounded failure guidance, next retry after failure, validated ZIP archives, download/import, and a select-review-exact-`RESTORE` maintenance flow. Importing a ZIP opens that same review flow. Superseded recurring failures remain historical but do not inflate actionable queue debt. The download comes from the authenticated server overview and uses the same selected cutoff and bounded sanitization as the screen. Dashboard and Printers do not duplicate detailed operational status; Cura Workstations retains the operator-facing Cura receipt summary.

## Print history, notifications, and accounts

Print History distinguishes exact-state records from unresolved legacy imports. Its top toolbar combines a printer selector defaulted to **All printers**, the outcome filter, a 10/25/50/100 per-page selector defaulted to 10, exact result range, and labelled First, Previous, Next, and Last controls backed by server-side newest-first pagination. Those browser query-string page sizes and printer IDs are explicitly validated. Repeating list responses omit the potentially large print-settings archive; opening one record retrieves that archive through its authenticated detail endpoint. A request failure shows a retryable unavailable state and must never masquerade as a genuine empty history. List/card records show the sanitized stored G-code thumbnail when available plus actual filament, captured cost, and the exact supported Moonraker outcome such as cancellation, Klippy shutdown, disconnection, or interruption. Completed, cancelled, and failed records use subtle green, yellow, and red semantic backgrounds respectively while retaining status text. Detail shows a larger thumbnail, immutable sliced and actual metadata, estimated-versus-actual duration and filament comparisons, complete/partial captured segment cost, complete-file hash, G-code inspection findings, spool/profile/plate snapshots, M600 segment weight/cost, and retained outcome history. **Advanced print settings** opens a grouped overlay for the complete resolved managed profile, its template at print time, semantic differences, and safely captured Cura global/per-extruder settings; formulas are displayed as inert text and unavailable legacy evidence is explicit. Operators can directly rate or update a finished print as failed, acceptable, successful, or excellent and add bounded defect tags and notes; earlier assessments remain immutable without exposing revision controls. The live Dashboard shows the same current-print thumbnail with progress, elapsed/estimated time, actual filament, and cost so far when exact data is available.

The application shell polls persistent notifications, shows unread severity and count, links to the affected workflow, supports individual or bulk read actions, and closes the notification panel after an outside pointer action without disrupting inside interactions. It shows the build version and direction-only collapse/restore chevrons, with no Logout action in the sidebar. Settings offers exactly three light and nine dark GUI color profiles. Activity cards use green for success, red for error, yellow for warning, and distinct semantic informational colors while retaining text/icon status cues. Settings contains one **Account** editor for username, display name, and password and no Security Defaults card. Active browser sessions receive a rolling seven-day idle window bounded by a fixed thirty-day absolute expiry. The empty-database `admin` / `admin` account sees only the forced password-replacement screen until changed.
...
The Printers page shows sanitized discovered Klipper/Moonraker versions, hostname, kinematics, nozzle, and build volume alongside editable hardware metadata. Information synchronizes automatically every 5 minutes while idle, while server-side operational state refreshes every 10 seconds while no print is active. Only Administrators may edit printer information, and no connection address or secret appears in the browser.
