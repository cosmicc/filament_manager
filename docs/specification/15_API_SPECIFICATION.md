...
The takeover request contains the complete reviewed source-ID set and explicit confirmation. It must contain no mappings. After validating the content-hashed source set, the server enables management and queues synchronization atomically without importing settings. POST /workstation-agents/{id}/sync is Administrator-only and queues a forced current-library refresh, superseding only unclaimed older library snapshots. Legacy Cura import endpoints reject requests; legacy managed-edit reports never mutate canonical settings.
