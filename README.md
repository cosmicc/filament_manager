...
- Cura-to-Fluidd spool preflight with bounded G-code/profile inspection, optional fail-closed blocking, guarded virtual-SD hold/release across printer start and build-plate selection, strict current exact-profile print choices, broader safe manual-load choices, and Spoolman updates only at completed unload/load boundaries
...
- A 5-second live Dashboard snapshot led by the full-width printer status card with three inventory value cards directly beneath it, plus safe Moonraker/Klipper availability, active spool and plate, print state and progress, current thumbnail, elapsed/estimated time, filament use and cost so far, and available nozzle, bed, and chamber temperatures
