...
- Canonical spool, filament, printer/nozzle-scoped print settings, printer, editable physical-nozzle codes, per-product multicolor/rainbow palettes, and compact responsive build-plate records with independently remembered List, Cards, and Detailed catalog views and complete selected-item actions
...
- Directly saved and automatically favorited `Template <material type>` Cura entries whose complete effective values flow only to current exact scopes that inherit them, except for visibly highlighted sparse customizations, with template-only speed/cooling/smooth-time/acceleration controls, profile-editable pressure advance, Initial Fan Speed and ironing, refreshed-before-edit concurrency protection, centered exact inline validation, one primary Flow, exactly Printing/Build Volume/Build Plate Temperature, and overlapping retract/fan aliases represented by one editor control
